...
# 2 "<built-in>" 2
# 1 "vector_add/vector_add.cpp" 2
__attribute__((sdx_kernel("vector_add", 0))) void vector_add(int *a, int *b, int *c)
{
#pragma HLS TOP name=vector_add
# 2 "vector_add/vector_add.cpp"
...
#pragma HLS INTERFACE m_axi depth=50 port=c
 ...
 int tmp_a[3], tmp_b[3], tmp_c[3];
    ...
    VITIS_LOOP_10_1: for (int i = 0; i < 3; i++)
    {
        tmp_a[i] = *(a+i);
        tmp_b[i] = *(b+i);
    }

    VITIS_LOOP_16_2: for (int i = 0; i < 3; i++)
    {
        tmp_c[i] = tmp_a[i] + tmp_b[i];
    }

    VITIS_LOOP_21_3: for (int i = 0; i < 3; i++)
    {
        *(c+i) = tmp_c[i];
    }
    return;
